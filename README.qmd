...
6. Prepare merged data to output. The output data is a move2 location object
comprising the entire content of the input data with the following add-ons:

    - *event data*: the list-colum `acc_dt` comprising the downloaded and merged 
    ACC events
    ...
    - *move2 attribute*: `acc_merging_rule` specifying the chosen merging 
    criteria (retrievable via `attr(output, "acc_merging_rule")`)
