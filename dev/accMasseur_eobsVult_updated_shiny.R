...
accMasseur_eobsVult_updated <- function(acc_dwnldDt){
  ...
  if(nrow(acc_dwnldDt_nested) > 0){
    ...
    acc_new_nested <- acc_dwnldDt_nested %>%
      ...
      mutate(hour = hour(eobs_start_timestamp),
             ...
             tag = as.character(deployment_id)) %>%
    ...
    acc_new_nested %<>%
      select(-c(contains("marked_outlier"), starts_with("acceleration_raw_"), data_decoding_software, contains("acceleration"), contains("orientation"),
  ...
  if(nrow(acc_dwnldDt_perRow)>0){
    ...
    acc_new_perRow <- acc_dwnldDt_perRow %>%
      ...
      group_by(individual_id, tag_id, deployment_id, timestamp_minFloor) %>%
      summarise(
